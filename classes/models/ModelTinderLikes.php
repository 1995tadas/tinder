...
class ModelTinderLikes {
    ...
    public function create() {
        $sql = "CREATE TABLE IF NOT EXISTS $this->table_name(" .
                ...
                "email_user_liked VARCHAR(255) NOT NULL," .
    ...
    public function insert($email_user, $email_user_liked) {
        $row = [
            'email_user' => $email_user,
            'email_user_liked' => $email_user_liked
    ...
    public function delete($email_user, $email_user_liked) {

        $sql = strtr('DELETE FROM @table WHERE @column_with_bind AND @column_liked_with_bind', [
            '@table' => $this->table_name,
            '@column_with_bind' => SQLBuilder::columnEqualBind('email_user'),
            '@column_liked_with_bind' => SQLBuilder::columnEqualBind('email_user_liked')
        ]);
        $query = $this->db_c->prepare($sql);
        $query->bindValue(SQLBuilder::bind('email_user'), $email_user);
